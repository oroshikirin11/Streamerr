...
export class TmdbMeta {
  ...
  _save() {
    ...
    this._saveTimer = setTimeout(() => {
      ...
      try {
        ...
        writeFileSync(tmp, JSON.stringify({ v: 1, entries: this._entries }));
  ...
  async ensure(kind, title, year) {
    ...
    const flight = (async () => {
      const y = year ?? titleYear(title);
      const q = normTitle(title);
      if (!q) return null;
      const params = { query: q };
      if (y) params[type === 'tv' ? 'first_air_date_year' : 'year'] = y;
      let found = this._pick((await this._get(`/search/${type}`, params)).results, title, y, type);
      if (!found && y) {
        found = this._pick((await this._get(`/search/${type}`, { query: q })).results, title, y, type);
      }
      if (!found) {
...
export class TmdbLibrary {
  ...
  _enrichSummary(it) {
    const e = this.tmdbMeta.lookup(it.type === 'Movie' ? 'movie' : 'tv', it.title, it.year);
    if (!e) return it;
    return {
      ...it,
